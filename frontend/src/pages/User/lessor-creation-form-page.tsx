...
const LessorCreationFormPage: FC<IPage> = ({ openSnackbar }): ReactElement => {
    const classes = useStyles('light');
    const navigate = useNavigate();
    const { handleErrorResponse } = useErrorHandling();
    const [creationCompleted, setCreationCompleted] = useState(false);

    const { data, error, postData } = usePost<ILessorDTO>('lessors', {});
    ...
    useEffect(() => {
        if (data !== null && creationCompleted) {
            openSnackbar('Lessor created successfully', 'success');
            navigate(-1);
        }
    ...
    const handleSaveChanges = async (
        ...
        const request: ILessorDTO = Object.keys(form).reduce(
...
export default LessorCreationFormPage;